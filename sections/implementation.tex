...
\begin{example}{Syntax of a small C language}{ex:c-spec}
  ...
  \[
    \begin{aligned}
      & \textit{p}       \in \textit{Prog}       \quad & \textit{b}       \in \textit{Block}        \\
      & \textit{bi}      \in \textit{BlockItem}  \quad & \textit{vd}      \in \textit{VarDecl}      \\
      & \textit{fd}      \in \textit{FunDecl}    \quad & \textit{s}       \in \textit{Stmt}         \\
      & \textit{pl}      \in \textit{ParamList}  \quad & \textit{e}       \in \textit{Exp}          \\
      & \textit{t}       \in \textit{Type}       \quad & \textit{id}      \in \textit{Id}           \\
      & \textit{int}     \in \textit{Int}        \quad & \textit{char}    \in \textit{Char}         \\
      & \textit{bool}    \in \textit{Bool}       \quad & \textit{cond}    \in \textit{Cond}         \\
   \end{aligned}
  \]
  ...
  \[
    \begin{aligned}
      ...
      & \textit{p} ::=      &  & \textit{b}                                 &  & \textit{(b)p}                             &  & \textit{prog}                \\
      & \textit{b} ::=      &  & \textit{"\{" bilist "\}" }                   &  & \textit{(bilist)b}                  &  & \textit{block}               \\
      & \textit{bilist} ::= &  & \textit{bi bilist}                         &  & \textit{(bi, bilist)bilist}               &  & \textit{blockitems}          \\
      & \quad |             &  & \textit{bi}                                &  & \textit{(bi)bilist}                       &  & \textit{singleblockitem}     \\
      & \textit{bi} ::=     &  & \textit{vd}                                &  & \textit{(vd)bi}                           &  & \textit{vardecl}             \\
      & \quad |             &  & \textit{s}                                 &  & \textit{(s)bi}                            &  & \textit{stmt}                \\
      & \quad |             &  & \textit{fd}                                &  & \textit{(fd)bi}                           &  & \textit{funcdecl}            \\
      & \textit{vd} ::=     &  & \textit{t id "=" e ";" }                   &  & \textit{(t, id, e)vd}                     &  & \textit{vardecl}             \\
      & \textit{fd} ::=     &  & \textit{t id "(" pl ")" "\{" b "\}" }        &  & \textit{(t, pl, id.b)fd}                 &  & \textit{funcdecl}            \\
      & \textit{pl} ::=     &  & \textit{param}                             &  & \textit{(param)pl}                        &  & \textit{paramlist}           \\
      & \quad |             &  & \textit{param "," pl}                      &  & \textit{(param, pl)pl}                    &  & \textit{params}              \\
      & \textit{param} ::=  &  & \textit{t id}                              &  & \textit{(t, id)param}                     &  & \textit{param}               \\
      & \textit{s} ::=      &  & \textit{id "=" e ";"}                      &  & \textit{(id, e)s}                         &  & \textit{assign}              \\
      & \quad |             &  & \textit{id "(" elist ");"}                 &  & \textit{(id, elist)s}                     &  & \textit{stmtfuncall}         \\
      & \quad |             &  & \textit{"return " e ";"}                   &  & \textit{(e)s}                             &  & \textit{returnstmt}          \\
      & \quad |             &  & \textit{cond}                              &  & \textit{(cond)s}                          &  & \textit{condstmt}            \\
      & \quad |             &  & \textit{b}                                 &  & \textit{(b)s}                             &  & \textit{blockstmt}           \\
      & \textit{elist} ::=  &  & \textit{e}                                 &  & \textit{(e)elist}                         &  & \textit{arg}                 \\
      & \quad |             &  & \textit{e "," elist}                       &  & \textit{(e, elist)elist}                  &  & \textit{args}                \\
      & \textit{cond} ::=   &  & \textit{"if (" e ") \{" b "\} else \{" b "\}"} &  & \textit{(e, b, b)cond}                   &  & \textit{ifelse}              \\
      & \textit{t} ::=      &  & \textit{"int"}                             &  & \textit{()t}                              &  & \textit{tint}                \\
      & \quad |             &  & \textit{"char"}                            &  & \textit{()t}                              &  & \textit{tchar}               \\
      & \quad |             &  & \textit{"bool"}                            &  & \textit{()t}                              &  & \textit{tbool}               \\
      & \textit{e} ::=      &  & \textit{int}                               &  & \textit{(int)e}                           &  & \textit{intexpr}             \\
      & \quad |             &  & \textit{char}                              &  & \textit{(char)e}                          &  & \textit{charexpr}            \\
      & \quad |             &  & \textit{bool}                              &  & \textit{(bool)e}                          &  & \textit{boolexpr}            \\
      & \quad |             &  & \textit{e "+" e}                           &  & \textit{(e, e)e}                          &  & \textit{plus}                \\
      & \quad |             &  & \textit{e "==" e}                          &  & \textit{(e, e)e}                          &  & \textit{equals}              \\
      & \quad |             &  & \textit{id "(" elist ")"}                  &  & \textit{(id, elist)e}                     &  & \textit{expfuncall}          \\
      & \quad |             &  & \textit{id}                                &  & \textit{(id)e}                            &  & \textit{expident}            \\
      & \textit{id} ::=     &  & \%\textit{string}                          &  & \textit{()id}                             &  & \textit{ident}               \\
      & \textit{int} ::=    &  & \%\textit{int}                             &  & \textit{()int}                            &  & \textit{intlit}              \\
      & \textit{char} ::=   &  & \%\textit{char}                            &  & \textit{()char}                           &  & \textit{charlit}             \\
      & \textit{bool} ::=   &  & \%\textit{bool}                            &  & \textit{()bool}                           &  & \textit{boollit}             \\
   \end{aligned}
  \]
